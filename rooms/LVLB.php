...
if ($row != false) {
	...
	$sql = $db->prepare('SELECT * FROM rooms WHERE LVLB = 1 AND sessionid = :id;');
	$sql->bindValue(':id', $curSessID);
	$visited = $sql->execute();
	...
	if($row == false) {
		...
		$q = $db->prepare('UPDATE rooms SET LVLB = 1 WHERE SESSIONID = :id;');
		...
		<div class="header">
			<div class="cont">
				...
				<div class="title_cont">
					<div class="title lrg">
						Ingkarni Wardli Basement <!-- Full Room Name (DESKTOP VIEW) -->
					</div>
					<div class="title sml">
						Basement Level <!-- Small Room Name (MOBILE VIEW) -->
					</div>
