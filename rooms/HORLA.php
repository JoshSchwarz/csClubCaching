...
<html>
<head>
	<title>Horace Lamb Lecture Theatre</title>
...
if ($row != false) {
	...
	$sql = $db->prepare('SELECT * FROM rooms WHERE HORLA = 1 AND sessionid = :id;');
	$sql->bindValue(':id', $curSessID);
	$visited = $sql->execute();
	...
	if($row == false) {
		...
		$q = $db->prepare('UPDATE rooms SET HORLA = 1 WHERE SESSIONID = :id;');
		...
		<div class="header">
			<div class="cont">
				...
				<div class="title_cont">
					<div class="title lrg">
						Horace Lamb Lecture Theatre <!-- Full Room Name (DESKTOP VIEW) -->
					</div>
					<div class="title sml">
						Horace Lamb <!-- Small Room Name (MOBILE VIEW) -->
					</div>
